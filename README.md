# PromptWeb Design Repository

## For humans — short intro

This repository is the **design source of truth** for the live PromptWeb site **The candela Studio**.

| | |
|---|---|
| **Live site** | https://akashmali.info/candela/ |
| **Design repo** | `Developergokul/akash_project_test` |
| **Branch** | `main` |
| **Plugin code** | `Akashmali6198/promptweb` (separate WordPress plugin — never mix into this repo) |

**AI agents:** always work **only** on repository `Developergokul/akash_project_test`. This value is written automatically from PromptWeb Settings when you Initialize / Re-initialize — agents must not use any other GitHub repo even if a later chat message forgets to name it.

What this repo is for:

- All website **page designs** (static HTML and dynamic PHP) used by the PromptWeb WordPress plugin
- **`pages/manifest.json`** — catalog of pages, status, and clean `public_url` values
- **AI instructions** — so any agent (with MCP tools **or** GitHub-only tools like ChatGPT) can build pages correctly

How the live site updates:

1. AI (or a human) commits page files + manifest to this design repo  
2. The WordPress PromptWeb plugin **syncs** from GitHub  
3. Pages appear at clean URLs: home `https://akashmali.info/candela/` · other pages `https://akashmali.info/candela/{slug}/`
...
├── static/             # Static HTML + Tailwind CDN + JS  →  pages/static/{slug}.html
└── dynamic/            # PHP + WordPress                 →  pages/dynamic/{slug}.php
AI_INSTRUCTIONS.md      # Complete full agent rules (A→Z)
README.md               # This file: short human intro + the same complete full agent rules
...
| **Dynamic** | `pages/dynamic/{slug}.php` | Forms, WordPress loops / queries / hooks |

### Dual instructions (Option B)

- **`AI_INSTRUCTIONS.md`** = complete full AI rules  
- **`README.md`** = short human intro **plus** the same complete full AI rules below  

**Both files contain the full rules** so any AI that reads **only one** file still gets everything.
...
# AI AGENTS — COMPLETE RULES (MANDATORY)

## SOURCE OF TRUTH NOTICE (mandatory)

This file (`README.md`) and `AI_INSTRUCTIONS.md` contain the **same complete rules** (dual full instructions — Option B).
...
1. **@GitHub Read README.md, AI_INSTRUCTIONS.md, and pages/manifest.json first.** Work only on repository: `Developergokul/akash_project_test`. (Mandatory opening step — load rules before creating pages.)
...
AI_INSTRUCTIONS.md   # Complete full agent rules
...
1. Read **this file** or **`AI_INSTRUCTIONS.md`** (both have full rules).
2. Detect Mode A or Mode B — never refuse if Mode B.
3. If reference URL and Mode A → `analyze_reference_url` first; if Mode B → best-effort without analyzer.
4. Rebuild full page only under allowed paths — not from existing site style when a reference is given.
5. Prefer extracted `image_urls`; else free Unsplash/SVG stand-ins — do not stop, do not ask for URLs.
6. Pass layout fidelity checklist → publish (tools or manifest) → commit → last line = exact `public_url`.
